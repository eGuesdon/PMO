/**
 * JiraInstanceManager – audit wiring & DB evidence test (sans lecture de fichier)
...
describe('JiraInstanceManager.initProjects – audit + DB', () => {
  beforeAll(() => {
    ...
    const transport = new FileAuditTransport('logs/audit.test.log');
  ...
  it('produit des évidences DB avec une liste de projets mockée', async () => {
    const inst = JiraInstanceManager.getInstance();
    ...
    const stubProjects = [
      {
        ...
        insight: { totalIssueCount: 3, lastIssueUpdateTime: new Date().toISOString() },
        lead: { displayName: 'Alice', active: true },
      },
      {
        id: '10002',
        key: 'OPS',
        ...
        lead: { displayName: 'Bob', active: false },
      },
    ] as any[];
    ...
    expect(projRows.map((r: any) => r.key)).toEqual(expect.arrayContaining(['DEMO', 'OPS']));
