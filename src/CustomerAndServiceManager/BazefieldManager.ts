import JiraServiceManager, { JiraProject, JiraInstanceField, JiraIssue } from '../JiraService/JiraServiceManager';
...
export default class BazefieldManager extends JiraServiceManager {
  ...
  private bzfData?: {
    ...
    issueTypes: Record<string, Array<{ id: string; name: string; hierarchyLevel?: number }>>;
  ...
  public async syncBazefieldData(): Promise<void> {
    ...
    const issueTypesByProject: Record<string, Array<{ id: string; name: string; hierarchyLevel?: number }>> = {};
    for (const p of targetProjects) {
      const types = (await (this as any).getIssueTypesForProject(p.id)) as Array<{ id: string; name: string; hierarchyLevel?: number }>;
    ...
    try {
      issues = await this.getIssues({
        ...
        fields: ['parent', 'issuetype', 'summary', 'issuelinks', 'status', 'updated', 'created'],
        maxResults: 10000,
      } as any);
    } catch {
      // fallback: par projet
    ...
    for (const issue of issues ?? []) {
      ...
      const parentKey = (issue as any)?.fields?.parent?.key || (issue as any)?.fields?.parent?.id;
      if (parentKey) {
        if (!parentChildren[parentKey]) parentChildren[parentKey] = [];
        const selfKey = (issue as any)?.key || (issue as any)?.id;
        if (selfKey) {
          parentChildren[parentKey].push(selfKey);
        }
      }
    }
